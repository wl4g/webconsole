...
$('#keyboard-paste').click(function () {
    ...
    navigator.clipboard.readText()
        .then(text => {
            alert(2)
            console.log("Clipboard read text of ", text)
            send(text);
        })
        .catch(err => {
            alert(3)
...
$('#keyboard-shift-left').click(function () {
...
$('#keyboard-shift-right').click(function () {
    send(b64_to_utf8("G1sxOzJD"));
});


$('#keyboard-ctrl').click(function () {
    if(ctrlLock) { // 若当前是已按下状态，则第二次按要复位
        releasedCtrl();
    } else {
        lockCtrl();
    }
...
function lockCtrl() {
    var ctrlBtn = $('#keyboard-ctrl');
    console.log($(ctrlBtn))
    $(ctrlBtn).addClass("ctrl-active");
    ctrlLock = true;
}
function releasedCtrl() {
    var ctrlBtn = $('#keyboard-ctrl');
    ctrlBtn.removeClass("ctrl-active");
    ctrlLock = false;
}
